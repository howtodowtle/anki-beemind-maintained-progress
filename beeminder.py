...
def as_datapoint(value, date, comment=None):
    if comment is None:
        comment = ""
    return {"value": value, "daystamp": date, "comment": comment, "requestid": date}
...
def add_datapoints(auth_token, goal_slug, datapoints):
    ...
    * optional "requestid"
    ...
    res = requests.post(
        url, data={"datapoints": json.dumps(datapoints), "auth_token": auth_token}
    )
    return _result_dict(res)
